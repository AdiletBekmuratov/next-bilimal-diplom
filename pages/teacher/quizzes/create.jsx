...
const CreateQuiz = ({ groups }) => {
  ...
  const handleSubmit = async () => {
    const data = {
      title: tempD.title,
      slug: slugify(`${tempD.title}-${Math.random()}`, {
        strict: true,
        lower: true,
      }),
      ...
      questions: tempD.questions.map((question) => ({
        Question_id: question,
      })),
    };

    await toast.promise(createQuiz(data, session?.user?.accessToken), {
  ...
  return (
    <>
      ...
      <AlertDialog
        ...
        description="Вы действительно хотите создать тест?"
      />
